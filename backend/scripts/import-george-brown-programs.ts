...
async function loadGeorgeBrownPrograms(): Promise<GeorgeBrownProgram[]> {
    try {
        const programsPath = path.join(__dirname, 'output', 'george_brown', 'georgebrown_programs_complete.json');
        const data = await fs.readFile(programsPath, 'utf-8');
        return JSON.parse(data);
    } catch (error) {
        console.error('Error loading George Brown programs:', error);
        return [];
    }
}

async function importBatch(programs: GeorgeBrownProgram[]): Promise<void> {
    try {
        const response = await axios.post(
            `${BACKEND_URL}/api/programs/bulk-import/george-brown`,
            { programs },
            {
                headers: {
...
async function importGeorgeBrownPrograms(): Promise<void> {
    console.log('🚀 Starting George Brown College programs import...\n');

    let totalSuccessful = 0;
    let totalErrors = 0;
    let totalProcessed = 0;

    const allPrograms = await loadGeorgeBrownPrograms();
    
    if (allPrograms.length === 0) {
        console.log('⚠️  No George Brown programs found to import');
        return;
    }

    console.log(`📊 Found ${allPrograms.length} George Brown programs to import`);

    const chunks: GeorgeBrownProgram[][] = [];
    ...
    for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
        ...
        console.log(`📤 Importing George Brown batch ${chunkIndex + 1}/${chunks.length} (${chunk.length} programs)...`);
        ...
        try {
            ...
            console.error(`❌ Failed to import George Brown batch ${chunkIndex + 1}`);
            totalErrors += chunk.length;
        }

        console.log(`✅ George Brown batch ${chunkIndex + 1} completed\n`);
    }

    console.log('🎉 George Brown College import process completed!');
...
async function main() {
    try {
        ...
        await importGeorgeBrownPrograms();
...
export { main as importGeorgeBrownPrograms }; 