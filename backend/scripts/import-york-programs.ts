...
async function loadYorkPrograms(): Promise<YorkProgram[]> {
    try {
        const programsPath = path.join(__dirname, 'output', 'yorkUni', 'yorkUni.json');
        const data = await fs.readFile(programsPath, 'utf-8');
        return JSON.parse(data);
    } catch (error) {
        console.error('Error loading York programs:', error);
        return [];
    }
}

async function importBatch(programs: YorkProgram[]): Promise<void> {
    try {
        const response = await axios.post(
            `${BACKEND_URL}/api/programs/bulk-import/york`,
            { programs },
            {
                headers: {
...
async function importYorkPrograms(): Promise<void> {
    console.log('🚀 Starting York University programs import...\n');

    let totalSuccessful = 0;
    let totalErrors = 0;
    let totalProcessed = 0;

    const allPrograms = await loadYorkPrograms();
    
    if (allPrograms.length === 0) {
        console.log('⚠️  No York programs found to import');
        return;
    }

    console.log(`📊 Found ${allPrograms.length} York programs to import`);

    const chunks: YorkProgram[][] = [];
    ...
    for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
        ...
        console.log(`📤 Importing York batch ${chunkIndex + 1}/${chunks.length} (${chunk.length} programs)...`);
        ...
        try {
            ...
            console.error(`❌ Failed to import York batch ${chunkIndex + 1}`);
            totalErrors += chunk.length;
        }

        console.log(`✅ York batch ${chunkIndex + 1} completed\n`);
    }

    console.log('🎉 York University import process completed!');
...
export { main as importYorkPrograms }; 