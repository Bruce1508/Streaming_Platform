...
async function loadTMUPrograms(): Promise<TMUProgram[]> {
    try {
        const programsPath = path.join(__dirname, 'output', 'tmu', 'tmu.json');
        const data = await fs.readFile(programsPath, 'utf-8');
        return JSON.parse(data);
    } catch (error) {
        console.error('Error loading TMU programs:', error);
        return [];
    }
}

async function importBatch(programs: TMUProgram[]): Promise<void> {
    try {
        const response = await axios.post(
            `${BACKEND_URL}/api/programs/bulk-import/tmu`,
            { programs },
            {
                headers: {
...
async function importTMUPrograms(): Promise<void> {
    console.log('🚀 Starting TMU programs import...\n');

    let totalSuccessful = 0;
    let totalErrors = 0;
    let totalProcessed = 0;

    const allPrograms = await loadTMUPrograms();
    
    if (allPrograms.length === 0) {
        console.log('⚠️  No TMU programs found to import');
        return;
    }

    console.log(`📊 Found ${allPrograms.length} TMU programs to import`);

    const chunks: TMUProgram[][] = [];
    ...
    for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
        ...
        console.log(`📤 Importing TMU batch ${chunkIndex + 1}/${chunks.length} (${chunk.length} programs)...`);
        ...
        try {
            ...
            console.error(`❌ Failed to import TMU batch ${chunkIndex + 1}`);
            totalErrors += chunk.length;
        }

        console.log(`✅ TMU batch ${chunkIndex + 1} completed\n`);
    }

    console.log('🎉 TMU import process completed!');
...
export { main as importTMUPrograms }; 