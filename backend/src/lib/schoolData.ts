interface CanadianSchool {
    name: string;
    code: string;
    type: 'University' | 'College' | 'Institute' | 'Seminary' | 'Cégep' | 'Specialized School' | 'Private College';
    province: string;
    location?: string;
    website?: string;
    established?: number;
    hasMedical?: boolean;
    hasDental?: boolean;
    isActive: boolean;
}

export const canadianSchools: CanadianSchool[] = [
    // Major Ontario Universities
    {
        name: "University of Toronto",
        code: "UTORONTO",
        type: "University",
        province: "Ontario",
        location: "Toronto, ON",
        website: "https://www.utoronto.ca",
        established: 1827,
        hasMedical: true,
        hasDental: true,
        isActive: true
    },
    {
        name: "McMaster University",
        code: "MCMASTER",
        type: "University",
        province: "Ontario",
        location: "Hamilton, ON",
        website: "https://www.mcmaster.ca",
        established: 1887,
        hasMedical: true,
        hasDental: false,
        isActive: true
    },
    {
        name: "Queen's University",
        code: "QUEENS",
        type: "University",
        province: "Ontario",
        location: "Kingston, ON",
        website: "https://www.queensu.ca",
        established: 1841,
        hasMedical: true,
        hasDental: false,
        isActive: true
    },
    {
        name: "University of Ottawa",
        code: "UOTTAWA",
        type: "University",
        province: "Ontario",
        location: "Ottawa, ON",
        website: "https://www.uottawa.ca",
        established: 1848,
        hasMedical: true,
        hasDental: false,
        isActive: true
    },
    {
        name: "Western University",
        code: "WESTERN",
        type: "University",
        province: "Ontario",
        location: "London, ON",
        website: "https://www.uwo.ca",
        established: 1878,
        hasMedical: true,
        hasDental: true,
        isActive: true
    },
    {
        name: "University of Waterloo",
        code: "WATERLOO",
        type: "University",
        province: "Ontario",
        location: "Waterloo, ON",
        website: "https://uwaterloo.ca",
        established: 1957,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "York University",
        code: "YORK",
        type: "University",
        province: "Ontario",
        location: "Toronto, ON",
        website: "https://www.yorku.ca",
        established: 1959,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Carleton University",
        code: "CARLETON",
        type: "University",
        province: "Ontario",
        location: "Ottawa, ON",
        website: "https://carleton.ca",
        established: 1942,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "University of Guelph",
        code: "GUELPH",
        type: "University",
        province: "Ontario",
        location: "Guelph, ON",
        website: "https://www.uoguelph.ca",
        established: 1964,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Brock University",
        code: "BROCK",
        type: "University",
        province: "Ontario",
        location: "St. Catharines, ON",
        website: "https://brocku.ca",
        established: 1964,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Trent University",
        code: "TRENT",
        type: "University",
        province: "Ontario",
        location: "Peterborough, ON",
        website: "https://www.trentu.ca",
        established: 1964,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Lakehead University",
        code: "LAKEHEAD",
        type: "University",
        province: "Ontario",
        location: "Thunder Bay, ON",
        website: "https://www.lakeheadu.ca",
        established: 1965,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Laurentian University",
        code: "LAURENTIAN",
        type: "University",
        province: "Ontario",
        location: "Sudbury, ON",
        website: "https://laurentian.ca",
        established: 1960,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "University of Windsor",
        code: "WINDSOR",
        type: "University",
        province: "Ontario",
        location: "Windsor, ON",
        website: "https://www.uwindsor.ca",
        established: 1963,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Wilfrid Laurier University",
        code: "WLU",
        type: "University",
        province: "Ontario",
        location: "Waterloo, ON",
        website: "https://www.wlu.ca",
        established: 1973,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Toronto Metropolitan University",
        code: "TMU",
        type: "University",
        province: "Ontario",
        location: "Toronto, ON",
        website: "https://www.torontomu.ca",
        established: 1948,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Ontario Tech University",
        code: "ONTARIOTECH",
        type: "University",
        province: "Ontario",
        location: "Oshawa, ON",
        website: "https://ontariotechu.ca",
        established: 2002,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "OCAD University",
        code: "OCAD",
        type: "University",
        province: "Ontario",
        location: "Toronto, ON",
        website: "https://www.ocadu.ca",
        established: 1876,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Nipissing University",
        code: "NIPISSING",
        type: "University",
        province: "Ontario",
        location: "North Bay, ON",
        website: "https://www.nipissingu.ca",
        established: 1992,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Algoma University",
        code: "ALGOMA",
        type: "University",
        province: "Ontario",
        location: "Sault Ste. Marie, ON",
        website: "https://algomau.ca",
        established: 1965,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },

    // Major Ontario Colleges
    {
        name: "Seneca Polytechnic",
        code: "SENECA",
        type: "College",
        province: "Ontario",
        location: "Toronto, ON",
        website: "https://www.senecapolytechnic.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "George Brown College",
        code: "GEORGEBROWN",
        type: "College",
        province: "Ontario",
        location: "Toronto, ON",
        website: "https://www.georgebrown.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Humber College",
        code: "HUMBER",
        type: "College",
        province: "Ontario",
        location: "Toronto, ON",
        website: "https://humber.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Sheridan College",
        code: "SHERIDAN",
        type: "College",
        province: "Ontario",
        location: "Oakville, ON",
        website: "https://www.sheridancollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Centennial College",
        code: "CENTENNIAL",
        type: "College",
        province: "Ontario",
        location: "Toronto, ON",
        website: "https://www.centennialcollege.ca",
        established: 1966,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Fanshawe College",
        code: "FANSHAWE",
        type: "College",
        province: "Ontario",
        location: "London, ON",
        website: "https://www.fanshawec.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Conestoga College",
        code: "CONESTOGA",
        type: "College",
        province: "Ontario",
        location: "Kitchener, ON",
        website: "https://www.conestogac.on.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Mohawk College",
        code: "MOHAWK",
        type: "College",
        province: "Ontario",
        location: "Hamilton, ON",
        website: "https://www.mohawkcollege.ca",
        established: 1966,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Algonquin College",
        code: "ALGONQUIN",
        type: "College",
        province: "Ontario",
        location: "Ottawa, ON",
        website: "https://www.algonquincollege.com",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "St. Lawrence College",
        code: "STLAWRENCE",
        type: "College",
        province: "Ontario",
        location: "Kingston, ON",
        website: "https://www.stlawrencecollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Durham College",
        code: "DURHAM",
        type: "College",
        province: "Ontario",
        location: "Oshawa, ON",
        website: "https://durhamcollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Fleming College",
        code: "FLEMING",
        type: "College",
        province: "Ontario",
        location: "Peterborough, ON",
        website: "https://flemingcollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Georgian College",
        code: "GEORGIAN",
        type: "College",
        province: "Ontario",
        location: "Barrie, ON",
        website: "https://www.georgiancollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Niagara College",
        code: "NIAGARA",
        type: "College",
        province: "Ontario",
        location: "Welland, ON",
        website: "https://www.niagaracollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "St. Clair College",
        code: "STCLAIR",
        type: "College",
        province: "Ontario",
        location: "Windsor, ON",
        website: "https://www.stclaircollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Cambrian College",
        code: "CAMBRIAN",
        type: "College",
        province: "Ontario",
        location: "Sudbury, ON",
        website: "https://cambriancollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Canadore College",
        code: "CANADORE",
        type: "College",
        province: "Ontario",
        location: "North Bay, ON",
        website: "https://www.canadorecollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Confederation College",
        code: "CONFEDERATION",
        type: "College",
        province: "Ontario",
        location: "Thunder Bay, ON",
        website: "https://confederationcollege.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Northern College",
        code: "NORTHERN",
        type: "College",
        province: "Ontario",
        location: "Timmins, ON",
        website: "https://www.northernc.on.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Sault College",
        code: "SAULT",
        type: "College",
        province: "Ontario",
        location: "Sault Ste. Marie, ON",
        website: "https://www.saultcollege.ca",
        established: 1965,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Loyalist College",
        code: "LOYALIST",
        type: "College",
        province: "Ontario",
        location: "Belleville, ON",
        website: "https://loyalistcollege.com",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Lambton College",
        code: "LAMBTON",
        type: "College",
        province: "Ontario",
        location: "Sarnia, ON",
        website: "https://www.lambtoncollege.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Collège Boréal",
        code: "BOREAL",
        type: "College",
        province: "Ontario",
        location: "Sudbury, ON",
        website: "https://www.collegeboreal.ca",
        established: 1995,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "La Cité collégiale",
        code: "LACITE",
        type: "College",
        province: "Ontario",
        location: "Ottawa, ON",
        website: "https://www.lacitec.on.ca",
        established: 1990,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },

    //MAJOR QUEBEC UNIVERSITIES

    {
        name: "McGill University",
        code: "MCGILL",
        type: "University",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.mcgill.ca",
        established: 1821,
        hasMedical: true,
        hasDental: true,
        isActive: true
    },
    {
        name: "Université Laval",
        code: "LAVAL",
        type: "University",
        province: "Quebec",
        location: "Québec, QC",
        website: "https://www.ulaval.ca",
        established: 1852,
        hasMedical: true,
        hasDental: true,
        isActive: true
    },
    {
        name: "Université de Montréal",
        code: "UMONTREAL",
        type: "University",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.umontreal.ca",
        established: 1878,
        hasMedical: true,
        hasDental: true,
        isActive: true
    },
    {
        name: "Université de Sherbrooke",
        code: "USHERBROOKE",
        type: "University",
        province: "Quebec",
        location: "Sherbrooke, QC",
        website: "https://www.usherbrooke.ca",
        established: 1954,
        hasMedical: true,
        hasDental: false,
        isActive: true
    },
    {
        name: "Concordia University",
        code: "CONCORDIA",
        type: "University",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.concordia.ca",
        established: 1974,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Bishop's University",
        code: "BISHOPS",
        type: "University",
        province: "Quebec",
        location: "Sherbrooke, QC",
        website: "https://www.ubishops.ca",
        established: 1843,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Polytechnique Montréal",
        code: "POLYMTL",
        type: "University",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.polymtl.ca",
        established: 1873,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "HEC Montréal",
        code: "HEC",
        type: "University",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.hec.ca",
        established: 1907,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },

    // Université du Québec System
    {
        name: "Université du Québec à Montréal",
        code: "UQAM",
        type: "University",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://uqam.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Université du Québec à Chicoutimi",
        code: "UQAC",
        type: "University",
        province: "Quebec",
        location: "Chicoutimi, QC",
        website: "https://www.uqac.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Université du Québec à Trois-Rivières",
        code: "UQTR",
        type: "University",
        province: "Quebec",
        location: "Trois-Rivières, QC",
        website: "https://www.uqtr.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Université du Québec à Rimouski",
        code: "UQAR",
        type: "University",
        province: "Quebec",
        location: "Rimouski, QC",
        website: "https://www.uqar.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Université du Québec en Outaouais",
        code: "UQO",
        type: "University",
        province: "Quebec",
        location: "Gatineau, QC",
        website: "https://uqo.ca",
        established: 1981,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Université du Québec en Abitibi-Témiscamingue",
        code: "UQAT",
        type: "University",
        province: "Quebec",
        location: "Rouyn-Noranda, QC",
        website: "https://www.uqat.ca",
        established: 1983,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "École de technologie supérieure",
        code: "ETS",
        type: "University",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.etsmtl.ca",
        established: 1974,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "TÉLUQ",
        code: "TELUQ",
        type: "University",
        province: "Quebec",
        location: "Québec, QC",
        website: "https://www.teluq.ca",
        established: 1972,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Institut national de la recherche scientifique",
        code: "INRS",
        type: "University",
        province: "Quebec",
        location: "Québec, QC",
        website: "https://inrs.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "École nationale d'administration publique",
        code: "ENAP",
        type: "University",
        province: "Quebec",
        location: "Québec, QC",
        website: "https://www.enap.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },

    // English Cégeps
    {
        name: "Dawson College",
        code: "DAWSON",
        type: "Cégep",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.dawsoncollege.qc.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "John Abbott College",
        code: "JOHNABBOTT",
        type: "Cégep",
        province: "Quebec",
        location: "Sainte-Anne-de-Bellevue, QC",
        website: "https://www.johnabbott.qc.ca",
        established: 1971,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Vanier College",
        code: "VANIER",
        type: "Cégep",
        province: "Quebec",
        location: "Saint-Laurent, QC",
        website: "https://www.vaniercollege.qc.ca",
        established: 1970,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Champlain Regional College",
        code: "CHAMPLAIN",
        type: "Cégep",
        province: "Quebec",
        location: "Sherbrooke, QC",
        website: "https://www.crc.qc.ca",
        established: 1971,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Heritage College",
        code: "HERITAGE",
        type: "Cégep",
        province: "Quebec",
        location: "Gatineau, QC",
        website: "https://www.Cégep-heritage.qc.ca",
        established: 1971,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Marianopolis College",
        code: "MARIANOPOLIS",
        type: "College",
        province: "Quebec",
        location: "Westmount, QC",
        website: "https://www.marianopolis.edu",
        established: 1908,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },

    // Major French Cégeps
    {
        name: "Cégep du Vieux Montréal",
        code: "CVMONTREAL",
        type: "Cégep",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.cvm.qc.ca",
        established: 1968,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Collège de Maisonneuve",
        code: "MAISONNEUVE",
        type: "Cégep",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.cmaisonneuve.qc.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Cégep de Sainte-Foy",
        code: "SAINTFOY",
        type: "Cégep",
        province: "Quebec",
        location: "Québec, QC",
        website: "https://www.Cégep-ste-foy.qc.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Collège François-Xavier-Garneau",
        code: "GARNEAU",
        type: "Cégep",
        province: "Quebec",
        location: "Québec, QC",
        website: "https://www.Cégepgarneau.ca",
        established: 1969,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Cégep de Saint-Laurent",
        code: "STLAURENT",
        type: "Cégep",
        province: "Quebec",
        location: "Saint-Laurent, QC",
        website: "https://www.Cégepsl.qc.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Collège de Rosemont",
        code: "ROSEMONT",
        type: "Cégep",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.crosemont.qc.ca",
        established: 1968,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Cégep de Sherbrooke",
        code: "CSHERBROOKE",
        type: "Cégep",
        province: "Quebec",
        location: "Sherbrooke, QC",
        website: "https://www.Cégepsherbrooke.qc.ca",
        established: 1968,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Cégep de Trois-Rivières",
        code: "TROISRIVIERES",
        type: "Cégep",
        province: "Quebec",
        location: "Trois-Rivières, QC",
        website: "https://www.Cégeptr.qc.ca",
        established: 1968,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Cégep Limoilou",
        code: "LIMOILOU",
        type: "Cégep",
        province: "Quebec",
        location: "Québec, QC",
        website: "https://www.Cégeplimoilou.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "Cégep Édouard-Montpetit",
        code: "EDMONTPETIT",
        type: "Cégep",
        province: "Quebec",
        location: "Longueuil, QC",
        website: "https://www.Cégepmontpetit.ca",
        established: 1967,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },

    // Specialized Institutions
    {
        name: "National Circus School",
        code: "CIRCUS",
        type: "Specialized School",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.ecolecirque.ca",
        established: 1981,
        hasMedical: false,
        hasDental: false,
        isActive: true
    },
    {
        name: "LaSalle College",
        code: "LASALLE",
        type: "Private College",
        province: "Quebec",
        location: "Montréal, QC",
        website: "https://www.lasallecollege.com",
        established: 1959,
        hasMedical: false,
        hasDental: false,
        isActive: true
    }
];


// Helper functions for migration/seeding
export const getSchoolsByProvince = (province: string): CanadianSchool[] => {
    return canadianSchools.filter(school => school.province === province);
};

export const getUniversities = (): CanadianSchool[] => {
    return canadianSchools.filter(school => school.type === 'University');
};

export const getColleges = (): CanadianSchool[] => {
    return canadianSchools.filter(school => school.type === 'College');
};