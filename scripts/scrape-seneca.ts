...
class SenecaScraper {
    ...
    private extractOverview($: cheerio.CheerioAPI): string {
        let overview = '';
    ...
    // Thêm method này vào class SenecaScraper, trước method scrapeAllPrograms()

    async scrapeNextBatch(startIndex: number = 50, batchSize: number = 50): Promise<void> {
        console.log(`🚀 Starting to scrape Seneca programs ${startIndex + 1}-${startIndex + batchSize}...`);
    ...
    async scrapeAllPrograms(): Promise<void> {
        ...
        try {
            ...
            const allProgramsData: ProgramData[] = [];
            ...
            // 3. Save tất cả data
            await fs.writeJSON(
                path.join(this.outputDir, 'processed', 'all_programs.json'),
                allProgramsData,
                { spaces: 2 }
            );

            console.log(`\n🎉 Scraping completed! Total programs: ${allProgramsData.length}`);
            console.log(`📁 Saved to: ${path.join(this.outputDir, 'processed', 'all_programs.json')}`);
...
async function main() {
    ...
    const scraper = new SenecaScraper();

    // Chọn một trong các options:
    // await scraper.testScrapeOneProgram();     // Test 1 program
    // await scraper.scrapeAllPrograms();       // Scrape batch đầu tiên (0-49)
        // await scraper.scrapeNextBatch(50, 50);   // Scrape batch tiếp theo (50-99)

    // Batch 3 (programs 100-149)
    // await scraper.scrapeNextBatch(100, 50);

    // Batch 4 (programs 150-199)  
